...
if($n_text < 4) {
  header("Location:/?text=".$text."&error=num1#tab2");
}
else {
  if($n_text > 200) {
   header("Location:/?text=".$text."&error=num2#tab2");
  }
  else {
...
if($auth==1) { // если юзер авторизован
  $sql5="INSERT INTO can(info, res, IP, date, user, privacy) VALUES ('$text', '$res', '$ip', NOW(), '$id', '$pri')";
} else {
  $sql5="INSERT INTO can(info, res, IP, date) VALUES ('$text', '$res', '$ip', NOW())";
}
